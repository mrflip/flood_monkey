...
class NetHTTPClientTest < Test::Unit::TestCase
  ...
  def test_that_using_auth_headers_on_get_requests_works_with_plaintext
    ...
    assert_equal 'GET', request.method
    assert_equal '/test?key=value', request.path
    correct_params = "oauth_nonce=\"225579211881198842005988698334675835446\", oauth_signature_method=\"PLAINTEXT\", oauth_token=\"token_411a7f\", oauth_timestamp=\"1199645624\", oauth_consumer_key=\"consumer_key_86cad9\", oauth_signature=\"5888bf0345e5d237%263196ffd991c8ebdb\", oauth_version=\"1.0\""
    auth_intro, auth_params = request['authorization'].split(' ', 2)
    assert_equal auth_intro, 'OAuth'
    assert_equal correct_params.split(', ').sort, auth_params.split(', ').sort
  end
  ...
  def test_that_using_post_with_uri_and_form_params_works
    ...
    assert_equal '/test', uri.path
    assert_equal nil, uri.fragment
    assert_equal "key=value&oauth_consumer_key=consumer_key_86cad9&oauth_nonce=225579211881198842005988698334675835446&oauth_signature=MrvEdxg%2BRvsw9sBcHbApZmjB%2BwM%3D&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1199645624&oauth_token=token_411a7f&oauth_version=1.0", uri.query.split("&").sort.join('&')
...
#    request = Net::HTTP::Get.new(request_uri.path + "?" + request_uri.query)
    ...
    assert_equal 'GET', request.method
    correct_params = 'oauth_nonce="kllo9940pd9333jh", oauth_signature_method="HMAC-SHA1", oauth_token="nnch734d00sl2jdk", oauth_timestamp="1191242096", oauth_consumer_key="dpf43f3p2l4k3l03", oauth_signature="tR3%2BTy81lMeYAr%2FFid0kMTYa%2FWM%3D", oauth_version="1.0"'.split(', ').sort
    correct_params.unshift 'OAuth realm="http://photos.example.net/"'
    assert_equal correct_params, request['authorization'].split(', ').sort
  end
