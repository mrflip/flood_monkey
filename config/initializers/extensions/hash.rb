...
class Hash
  ...
  def inflect_keys inflection
    infl_hash = {}
    self.each do |key, val|
      key = key.to_s.send(inflection).to_sym
      val = val.inflect_keys(inflection) if val.respond_to?(:inflect_keys)
      infl_hash[key] = val
    end
    infl_hash
  end
  ...
  def underscore_keys
    inflect_keys :underscore
  end
  def camelize_keys
    inflect_keys :camelize
  end
end
